...
class Reward:
    ...
    def reward_function(self, params):
        ...
        if steering_angle > ABS_STEERING_THRESHOLD:
            reward *= 0.9
            print("Too much steering: %f" % steering_angle)
        ...
        We cap difference to be within -MAX_DIFF and +MAX_DIFF to prevent extreme values.
        This ensures that the normalized difference stays within -1 and 1.
        Normalization:

        By dividing by MAX_DIFF, we normalize difference to a manageable range.
        Cubic Function:

        Cubing the normalized difference keeps the cubic relationship while controlling the magnitude.
        Scaling the Reward:

        Multiplying by 50 (the base reward) scales the cubic component appropriately.
        Example Calculation:

        When steps = 375 and LEAST_STEPS = 270:

        difference = 375 - 270 = 105
        difference is capped to MAX_DIFF = 100, so difference = 100
        normalized_diff = 100 / 100 = 1
        cubic_component = 1 ** 3 = 1
        reward += 50 - (50 * 1) = 0
        When steps = 250 and LEAST_STEPS = 270:

        difference = 250 - 270 = -20
        normalized_diff = -20 / 100 = -0.2
        cubic_component = (-0.2) ** 3 = -0.008
        reward += 50 - (50 * -0.008) = 50 + 0.4 = 50.4
