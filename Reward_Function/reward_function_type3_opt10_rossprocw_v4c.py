...
class Reward:
    ...
    def reward_function(self, params):
        ...
        if all_wheels_on_track and not is_offtrack:
            ...
            # Initialize variable to store the maximum angle difference
            max_angle_diff = 0.0

            # Loop through the range from LOOKAHEAD_START to LOOKAHEAD_END
            for i in range(LOOKAHEAD_START, LOOKAHEAD_END + 1):
                ...
                # For the first iteration, use the car's current heading as previous heading
                ...
                # Store the current waypoint heading for the next iteration
                prev_waypoint_heading = waypoint_heading

            # If the maximum angle difference is less than or equal to 10 degrees
            if max_angle_diff <= 10.0:
                # Penalize steering angles greater than 10 degrees
                STEERING_THRESHOLD = 10.0  # degrees
                ...
                SPEED_THRESHOLD = optimals[2]  # Encourage speed higher than optimal
