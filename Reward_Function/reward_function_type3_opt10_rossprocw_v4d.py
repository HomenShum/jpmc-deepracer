...
class Reward:
    ...
    def reward_function(self, params):
        ...
        ### 10/02/2024 TODO: Reward and Punish for the high degrees turn track sections as well. ###
        """
        Key Idea: For high-degree turn sections (where turns are sharp, e.g., >30 degrees), 
        the goal is to reward the car if it manages to navigate through these turns efficiently 
        (good steering and maintaining optimal speed) and penalize it if the steering is excessive or 
        the car slows down too much.
        Key Idea: For straight-line sections (where essentially no steering is required), 
        reward the car for maintaining high speed and minimal steering. 
        This encourages the car to maximize speed in straight sections without unnecessary steering adjustments.
        """
        ...
        if all_wheels_on_track and not is_offtrack:
            ...
            # Define the start and end of the lookahead range
            LOOKAHEAD_START = 3
            LOOKAHEAD_END = 7  # inclusive

            # Get the current closest waypoint index
            next_waypoint_index = closest_waypoints[1]

            ################### Reward for Optimal Steering Before Turn #########################

            # Look ahead to see if a significant turn (> 5 degrees) is approaching
            LOOKAHEAD_TURN_THRESHOLD = 5  # degrees, if the upcoming turn is greater than this
            STEERING_ANGLE_BEFORE_TURN_THRESHOLD = 3  # degrees, optimal steering threshold

            # Check the angle difference between waypoints in the look-ahead range
            turn_angle_detected = False
            for i in range(LOOKAHEAD_START, LOOKAHEAD_END + 1):
                ...
                # Calculate the heading between waypoints
                ...
                # Calculate angle difference between current and next waypoint heading
                angle_diff = abs(waypoint_heading - heading_prev)
                if angle_diff > 180:
                    angle_diff = 360 - angle_diff

                # Detect if a turn is approaching
                if angle_diff > LOOKAHEAD_TURN_THRESHOLD:
                    turn_angle_detected = True
                    break  # Exit loop as a turn has been detected

                prev_waypoint_heading = waypoint_heading

            # If a turn is detected, check if the steering angle is optimized
            if turn_angle_detected:
                if abs(steering_angle) <= STEERING_ANGLE_BEFORE_TURN_THRESHOLD:
                    reward += 1.2  # Reward for optimal steering input before the turn
                    if self.verbose:
                        print(f"Optimal steering before turn. Steering angle: {steering_angle}")
                else:
                    reward *= 0.8  # Penalize if steering is too aggressive
                    if self.verbose:
                        print(f"Oversteering detected before turn. Steering angle: {steering_angle}")

            #####################################################################################

            ################### Reward and Punishment for High-Degree Turns #########################

            HIGH_TURN_THRESHOLD = 30  # degrees, threshold to detect sharp turns
            OPTIMAL_STEERING_FOR_HIGH_TURN = 15  # degrees, optimal steering for sharp turns
            MIN_SPEED_FOR_HIGH_TURN = 1.5  # m/s, minimum speed in sharp turns to avoid penalty

            # Check the angle difference between consecutive waypoints to detect high-degree turns
            for i in range(LOOKAHEAD_START, LOOKAHEAD_END + 1):
                waypoint_index = (next_waypoint_index + i) % len(waypoints)
                prev_waypoint_index = (waypoint_index - 1) % len(waypoints)
                ...
                # Calculate the angle difference between the current and next waypoint heading
                angle_diff = abs(waypoint_heading - heading_prev)
                if angle_diff > 180:
                    angle_diff = 360 - angle_diff

                # Detect if a high-degree turn is occurring
                if angle_diff > HIGH_TURN_THRESHOLD:
                    # Reward for optimal steering
                    if abs(steering_angle) <= OPTIMAL_STEERING_FOR_HIGH_TURN:
                        reward += 1.5  # Reward for optimal steering in sharp turn
                        if self.verbose:
                            print(f"Optimal steering for high-degree turn. Steering angle: {steering_angle}")
                    else:
                        reward *= 0.7  # Penalize for oversteering
                        if self.verbose:
                            print(f"Oversteering in high-degree turn. Steering angle: {steering_angle}")
                    
                    # Penalize for low speed in sharp turns
                    if speed < MIN_SPEED_FOR_HIGH_TURN:
                        reward *= 0.8  # Penalize for slowing down too much
                        if self.verbose:
                            print(f"Speed too low for high-degree turn. Speed: {speed}")
                    
                    break  # Exit loop once a high-degree turn is detected

            #########################################################################################


            ################### Reward for Straight-Line Sections #########################

            STRAIGHT_LINE_THRESHOLD = 5  # degrees, threshold for detecting straight sections
            HIGH_SPEED_FOR_STRAIGHT = 3.5  # m/s, optimal speed for straight sections
            STEERING_PENALTY_FOR_STRAIGHT = 5  # degrees, penalty if steering exceeds this

            # Check if the section ahead is a straight line (small angle differences)
            straight_section_detected = True
            for i in range(LOOKAHEAD_START, LOOKAHEAD_END + 1):
                waypoint_index = (next_waypoint_index + i) % len(waypoints)
                prev_waypoint_index = (waypoint_index - 1) % len(waypoints)
                ...
                # If any angle exceeds the straight-line threshold, it's not a straight section
                if angle_diff > STRAIGHT_LINE_THRESHOLD:
                    straight_section_detected = False
                    break

                prev_waypoint_heading = waypoint_heading

            # If a straight-line section is detected
            if straight_section_detected:
                # Reward for maintaining high speed
                if speed >= HIGH_SPEED_FOR_STRAIGHT:
                    reward += 1.5  # Reward for high speed in straight section
                    if self.verbose:
                        print(f"High speed on straight section. Speed: {speed}")
                
                # Penalize for unnecessary steering
                if abs(steering_angle) > STEERING_PENALTY_FOR_STRAIGHT:
                    reward *= 0.8  # Penalize for unnecessary steering on straight sections
                    if self.verbose:
                        print(f"Unnecessary steering on straight section. Steering angle: {steering_angle}")
